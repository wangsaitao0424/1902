...
coin02.png" /><span><a href="">首页</a>&nbsp;-&nbsp;<a
		...
		<div class="page ">
			...
			<div class="banneradd bor">
				...
				<form  method="post" action="{{url('cat/update/'.$data->cat_id.'/'.$data->parent_id)}}">
				@csrf
				<div class="baBody">
					<div class="bbD">
						分类名称：<input type="text" name="cat_name" value="{{$data->cat_name}}" class="input3" />@php echo $errors->first('cat_name') @endphp
					</div>
					<div class="bbD">
						话题时长：<select class="input3" name="parent_id">
										<option value="{{$data->parent_id}}">{{$data->cat_name}}</option>
										<option value="0">顶级分类</option>
										@foreach ($cat as $v)
										<option value="{{$v->parent_id}}">{{str_repeat('——|  ',$v->level-1).$v->cat_name}}</option>
										@endforeach
								  </select>
					</div>
					<!-- <div class="bbD">
					...
					<div class="bbD">
						分类排序：<input type="number" name="sort_order" value="{{$data->sort_order}}" class="input3" />@php echo $errors->first('sort_order') @endphp
					</div>
					<div class="bbD">
						<p class="bbDP">
